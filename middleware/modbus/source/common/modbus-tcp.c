...
#define CAST    (char *)
#endif
    return send(ctx->s, CAST req, req_length, MSG_NOSIGNAL);
}
...
static int _modbus_tcp_set_ipv4_options(int s)
{
    ...
    /* SOL_TCP = IPPROTO_TCP */
    option = 1;
    rc = setsockopt(s, SOL_TCP, OPT_NO_NAGLE_ALGORITHM, (const void *)&option, sizeof(int32_t));
    if (rc == -1) {
        return -1;
    }

    option = 1000; /* TCP connection timeout (ms) */
    rc = setsockopt(s, SOL_TCP, OPT_CONNECT_TIMEOUT, (const void *)&option, sizeof(int32_t));
    if (rc == -1) {
        return -1;
    }
#endif
...
void _modbus_tcp_close(modbus_t *ctx)
{
    ...
    close(ctx->s);
#else // _MQX
    shutdownsocket(ctx->s, SHUT_RDWR);
    closesocket(ctx->s);
#endif
...
#else // _MQX
    uint32_t ms_timeout = 0; /* Wait indefinitely */

    if (tv != NULL) {
        ms_timeout = (tv->tv_sec * 1000) + (tv->tv_usec / 1000);
    }

    s_rc = select(ctx->s+1, rfds, NULL, NULL, ms_timeout);
    if (s_rc == RTCS_ERROR) {
        if (RTCS_errno == MQX_EBADF) {
            errno = EBADF;
        }
        return -1;
    }
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
    ...
    ctx->backend_data = (modbus_tcp_t *)_mem_alloc_system(sizeof(modbus_tcp_t));
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    ctx->backend_data = (modbus_tcp_pi_t *)_mem_alloc_system(sizeof(modbus_tcp_pi_t));
    if (ctx->backend_data == NULL) {
        modbus_free(ctx);
        errno = ENOMEM;
        return NULL;
