...
int _sleep_and_flush(modbus_t *ctx)
{
    ...
    while (nanosleep(&request, &remaining) == -1 && errno == EINTR)
        request = remaining;
#else // _MQX
    _time_delay((ctx->response_timeout.tv_sec * 1000) +
            (ctx->response_timeout.tv_usec / 1000));
#endif
...
static int receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    FD_SET(ctx->s, &rfds);
#else // _MQX
    /* Workaround: MQX does not support select() and FD_xxx macros
       for serial I/O.
     */
...
#else // _MQX
    mb_mapping = (modbus_mapping_t *)_mem_alloc_system(sizeof(modbus_mapping_t));
#endif
    if (mb_mapping == NULL) {
        return NULL;
    }
...
#else // _MQX
        mb_mapping->tab_bits =
                (uint8_t *) _mem_alloc_system(nb_bits * sizeof(uint8_t));
        if (mb_mapping->tab_bits == NULL) {
            _mem_free(mb_mapping);
            return NULL;
        }
#endif
...
#else // _MQX
        mb_mapping->tab_input_bits =
                (uint8_t *) _mem_alloc_system(nb_input_bits * sizeof(uint8_t));
        if (mb_mapping->tab_input_bits == NULL) {
            _mem_free(mb_mapping->tab_bits);
            _mem_free(mb_mapping);
            return NULL;
        }
#endif
...
#else // _MQX
        mb_mapping->tab_registers =
                (uint16_t *) _mem_alloc_system(nb_registers * sizeof(uint16_t));
        if (mb_mapping->tab_registers == NULL) {
            _mem_free(mb_mapping->tab_input_bits);
            _mem_free(mb_mapping->tab_bits);
            _mem_free(mb_mapping);
            return NULL;
        }
...
#else // _MQX
        mb_mapping->tab_input_registers =
                (uint16_t *) _mem_alloc_system(nb_input_registers * sizeof(uint16_t));
        if (mb_mapping->tab_input_registers == NULL) {
            _mem_free(mb_mapping->tab_registers);
            _mem_free(mb_mapping->tab_input_bits);
            _mem_free(mb_mapping->tab_bits);
            _mem_free(mb_mapping);
            return NULL;
...
void modbus_mapping_free(modbus_mapping_t *mb_mapping)
{
    ...
    free(mb_mapping);
#else // _MQX
    _mem_free(mb_mapping->tab_input_registers);
    _mem_free(mb_mapping->tab_registers);
    _mem_free(mb_mapping->tab_input_bits);
    _mem_free(mb_mapping->tab_bits);
    _mem_free(mb_mapping);
