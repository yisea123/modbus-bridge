...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    speed_t speed;
#else // _MQX
    NIO_SERIAL_INIT_DATA_STRUCT nio_serial_init;
#endif
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
...
#else // _MQX
    char *p = ctx_rtu->device;
    while (*p != '\0' && !isdigit((unsigned char)*p)) {
        p++;
    }
    nio_serial_init.SERIAL_INSTANCE = (*p != '\0') ? (*p - '0') : 0;

    nio_serial_init.CLK_SOURCE = 1;
    nio_serial_init.RXTX_PRIOR = 4;
    nio_serial_init.MODULE = kNioSerialUart;
    nio_serial_init.RX_BUFF_SIZE = NIO_SERIAL_BUFF_SIZE;
    nio_serial_init.TX_BUFF_SIZE = NIO_SERIAL_BUFF_SIZE;

    if (ctx_rtu->parity == 'N') {
        nio_serial_init.PARITY_MODE = kNioSerialParityDisabled;
    } else if (ctx_rtu->parity == 'E') {
        nio_serial_init.PARITY_MODE = kNioSerialParityEven;
    } else {
        // Odd
        nio_serial_init.PARITY_MODE = kNioSerialParityOdd;
    }

    nio_serial_init.BAUDRATE = ctx_rtu->baud;
    switch (ctx_rtu->data_bit) {
    case 5:
        nio_serial_init.BITCOUNT_PERCHAR = (ctx_rtu->parity == 'N') ? 5 : 6;
        break;
    case 6:
        nio_serial_init.BITCOUNT_PERCHAR = (ctx_rtu->parity == 'N') ? 6 : 7;
        break;
    case 7:
        nio_serial_init.BITCOUNT_PERCHAR = (ctx_rtu->parity == 'N') ? 7 : 8;
        break;
    case 8:
    default:
        nio_serial_init.BITCOUNT_PERCHAR = (ctx_rtu->parity == 'N') ? 8 : 9;
    }

    /* Stop bit (1 or 2) */
    if (ctx_rtu->stop_bit == 1) {
        nio_serial_init.STOPBIT_COUNT = kNioSerialOneStopBit;
    } else { /* 2 */
        nio_serial_init.STOPBIT_COUNT = kNioSerialTwoStopBit;
    }

    _nio_dev_install(ctx_rtu->device, &nio_serial_dev_fn, (void *)&nio_serial_init, NULL);

    ctx->s = open(ctx_rtu->device, 0);
    if (ctx->s < 0) {
        return -1;
    }
...
#else
    close(ctx->s);
    _nio_dev_uninstall(ctx_rtu->device, NULL);
#endif
}
...
#else
    return ioctl(ctx->s, IO_IOCTL_FLUSH_OUTPUT, NULL);
#endif
}
        ...
        struct timeval *tv, int length_to_read)
{
...
#else // _MQX
    /* select() for serial I/O operation is not supported in Freescale MQX */
    uint32_t ms_timeout = 0, bytes;
    bool block = false;
    ...
    if (tv == NULL) {
        ...
        ms_timeout = (tv->tv_sec * 1000) + (tv->tv_usec / 1000);
    }

    while ((s_rc = ioctl(ctx->s, IO_IOCTL_CHAR_AVAIL, &bytes)) == 0 && bytes < length_to_read) {
        if (block) {
            _time_delay(1); /* Block until some bytes arrived */
        } else {
            if (ms_timeout > 0) {
                _time_delay(1);
                ms_timeout--;
        ...
        int stop_bit)
{
    ...
    ctx = (modbus_t *)_mem_alloc_system(sizeof(modbus_t));
    if (ctx == NULL) {
        errno = ENOMEM;
        return NULL;
    }
    ...
    ctx->backend_data = (modbus_rtu_t *)_mem_alloc_system(sizeof(modbus_rtu_t));
